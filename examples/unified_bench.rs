...
impl CpuTimes {
    ...
    fn total(&self) -> u64 { self.user + self.nice + self.system + self.idle + self.iowait + self.irq + self.softirq + self.steal }
    fn busy(&self) -> u64 { self.total() - self.idle - self.iowait }
}
...
struct LatencyTracker { buf: Vec<u64>, pos: usize }

impl LatencyTracker {
    fn new(cap: usize) -> Self { Self { buf: Vec::with_capacity(cap), pos: 0 } }
    fn record(&mut self, us: u64) {
        if self.buf.len() < self.buf.capacity() { self.buf.push(us); } else { self.buf[self.pos] = us; }
        self.pos = (self.pos + 1) % self.buf.capacity();
    }
    fn stats(&self) -> (f64, u64, u64, u64) {
        if self.buf.is_empty() { return (0.0, 0, 0, 0); }
        let mut s = self.buf.clone(); s.sort_unstable();
        let avg = s.iter().sum::<u64>() as f64 / s.len() as f64;
        (avg, s[s.len()/2], s[(s.len() as f64 * 0.99) as usize], *s.last().unwrap())
    }
    fn clear(&mut self) { self.buf.clear(); self.pos = 0; }
}

fn fmt_mb(b: usize) -> String { format!("{:.1}", b as f64 / (1024.0 * 1024.0)) }
fn fmt_us(us: u64) -> String { if us >= 1000 { format!("{:.1}ms", us as f64/1000.0) } else { format!("{}us", us) } }
...
fn percent_encode(bytes: [u8; 20]) -> String {
    ...
    for &b in &bytes {
        if b.is_ascii_alphanumeric() || b == b'-' || b == b'_' || b == b'.' || b == b'~' { s.push(b as char); }
        else { s.push('%'); s.push(hex_digit(b>>4)); s.push(hex_digit(b&0xf)); }
    } s
}
fn hex_digit(n: u8) -> char { match n { 0..=9 => (b'0'+n) as char, 10..=15 => (b'a'+n-10) as char, _ => '?' } }
fn make_info_hash(t: usize) -> [u8; 20] { let mut h=[0u8;20]; h[0..8].copy_from_slice(&(t as u64).to_le_bytes()); h }
fn make_peer_id(i: usize) -> [u8; 20] { let mut p=[0u8;20]; p[0..8].copy_from_slice(&(i as u64).to_le_bytes()); p }
...
impl Rng {
    ...
    fn next_usize(&mut self, b: usize) -> usize { if b==0 {0} else {(self.next_u64() as usize) % b} }
}
...
async fn main() {
    ...
    let app = router(AppState::sharded(
        Duration::from_secs(1800), Duration::from_secs(2700), 16,
    ));
    ...
    for _ in 0..total_requests {
        ...
        let (uri, torrent_idx, port) = if is_new {
            ...
            (announce_uri(make_info_hash(ti), make_peer_id(idx), p, left), ti, p)
        };

        if is_new { next_peer_idx += 1; new_join_count += 1; } else { reannounce_count += 1; }
        request_count += 1;
        ...
        if request_count >= next_sample {
            ...
            } else { prev_cpu = CpuTimes::read(); 0.0 };
