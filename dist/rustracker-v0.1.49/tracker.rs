...
impl Tracker {
    ...
    pub fn client_distribution(&self) -> Vec<(u8, u64)> {
        let mut total: Vec<(u8, u64)> = Vec::new();
        for swarm in self.swarms.values() {
            for &(tag, count) in &swarm.client_counts {
                if count > 0 {
                    match total.iter_mut().find(|(t, _)| *t == tag) {
                        Some((_, c)) => *c += count as u64,
                        None => total.push((tag, count as u64)),
                    }
                }
            }
        }
...
impl Swarm {
    ...
    fn upsert_peer(&mut self, endpoint: PeerEndpoint, peer: PeerState) {
        ...
        if let Some(old_peer) = old {
            ...
            self.decr_client(old_peer.client_tag);
        }

        self.incr_client(new_tag);
    ...
    fn remove_peer(&mut self, endpoint: PeerEndpoint) {
        ...
        if let Some(peer) = removed {
            self.decr_client(peer.client_tag);
