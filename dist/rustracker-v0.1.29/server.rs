...
<body>
    <div class="app">
        <aside class="side">
            ...
            <div class="nav-item"><span>Tracker 概览</span><span id="navState">运行中</span></div>
            <a class="nav-item nav-link" href="#disclaimer"><span>Tracker 免责说明</span><span>查看</span></a>
            <p class="side-note">HTTP tracker 的连接、做种、下载和完成统计。</p>
        </aside>
        ...
        <main class="main">
            <section class="topbar">
                <div>
                    <h1>Tracker 控制台</h1>
                    <p class="subtle">查看当前端口上的 peer、做种和下载状态。</p>
                    <div class="status-line" id="statusText">正在加载...</div>
                    <div class="status-line" id="configText">上报间隔：- | 超时：-</div>
                </div>
                <div class="actions">
                    <button class="button secondary" id="pauseBtn" type="button">暂停刷新</button>
                    <button class="button" id="refreshBtn" type="button">刷新</button>
                </div>
            </section>
            ...
            <section class="chart-panel" aria-label="Tracker 趋势图">
                <div class="chart-head">
                    <h2 class="chart-title">Tracker 趋势</h2>
                    <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                        <span class="chart-note">Torrents、Peers、Seeders 和 Leechers 随时间变化</span>
                        <div class="range-group" id="rangeGroup">
                            <button class="range-btn active" data-range="24h" type="button">24小时</button>
                            <button class="range-btn" data-range="3d" type="button">3天</button>
                            <button class="range-btn" data-range="7d" type="button">7天</button>
                        </div>
                    </div>
                </div>
            ...
            <section class="disclaimer" id="disclaimer" aria-label="Tracker 免责说明">
                <h2>Tracker 免责说明</h2>
                <p>本站 Tracker 仅提供连接协调、状态记录与统计展示，不存储、不托管、不分发任何实际资源内容。</p>
                <p>页面中的 torrents、peers、seeders、leechers、客户端类型及趋势数据，来源于客户端上报与系统采样，可能存在延迟、缺失、偏差或伪造，不代表资源真实状态。</p>
                <p>本页面信息不代表任何资源的真实性、完整性、可用性、安全性或合法性，也不构成任何服务承诺或结果保证。</p>
                <p>对于第三方客户端行为、资源内容、传输结果及由此产生的任何直接或间接后果，本站不承担责任，使用者应自行判断并承担相关风险。</p>
                <p>受 Tracker 工作机制限制，本站不保留可用于长期识别、追踪或还原单个连接历史的完整日志，也无法对既往连接行为提供持续、完整或可验证的回溯记录。</p>
                <div class="disclaimer-contact">
                    <div>Blog：<a href="https://blog.7471.top/" rel="noopener noreferrer" target="_blank">https://blog.7471.top/</a></div>
                    <div>如有问题请联系：<a href="mailto:tracker@mail.7471.top">tracker@mail.7471.top</a></div>
                </div>
            </section>
        </main>
    </div>
    ...
    <script>
        const state = { data: null, paused: false, range: "24h" };
        ...
        function setStatus(text, error = false) {
            ...
            $("navState").textContent = error ? "异常" : state.paused ? "已暂停" : "运行中";
        }
        ...
        async function loadStats() {
            try {
                ...
                render();
                setStatus(`最后更新 ${new Date().toLocaleTimeString("zh-CN")}`);
            } catch (error) {
        ...
        function render() {
            ...
            $("configText").textContent = `上报间隔：${data.interval || "-"} 秒 | 超时：${data.peer_timeout || "-"} 秒`;
            renderChart();
        }
        ...
        function renderChart() {
            ...
            chart.setOption({
                ...
                grid: { left: 44, right: 20, top: 52, bottom: 36 },
                xAxis: {
                    type: "category",
                    boundaryGap: false,
                    data: labels,
        ...
        $("pauseBtn").addEventListener("click", () => {
            ...
            setStatus(state.paused ? "自动刷新已暂停" : "自动刷新已恢复");
