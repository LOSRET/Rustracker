...
struct Swarm {
    ...
    incomplete: usize,
    downloaded: u64,
    client_counts: [u32; 256],
}

impl Default for Swarm {
    fn default() -> Self {
        Self {
            ipv4_peers: PackedIpv4Peers::default(),
            ipv6_peers: PackedIpv6Peers::default(),
            complete: 0,
            incomplete: 0,
            downloaded: 0,
            client_counts: [0u32; 256],
        }
    }
}
...
impl Tracker {
    ...
    pub fn announce(&mut self, input: AnnounceInput, now: Instant) -> AnnounceOutput {
        ...
        let was_complete = swarm.peer_state(endpoint).map(|peer| peer.is_complete());
    ...
    pub fn snapshot(&self) -> TrackerSnapshot {
        ...
        let clients = self.client_distribution();
    ...
    pub fn client_distribution(&self) -> Vec<(u8, u64)> {
        let mut total = [0u64; 256];
        for swarm in self.swarms.values() {
            for (tag, &count) in swarm.client_counts.iter().enumerate() {
                if count > 0 {
                    total[tag] += count as u64;
                }
            }
        }
        total
...
impl Swarm {
    ...
    fn upsert_peer(&mut self, endpoint: PeerEndpoint, peer: PeerState) {
        ...
        if let Some(old_peer) = old {
            ...
            self.client_counts[old_peer.client_tag as usize] =
                self.client_counts[old_peer.client_tag as usize].saturating_sub(1);
        }

        self.client_counts[new_tag as usize] =
            self.client_counts[new_tag as usize].saturating_add(1);
    ...
    fn remove_peer(&mut self, endpoint: PeerEndpoint) {
        ...
        if let Some(peer) = removed {
            self.client_counts[peer.client_tag as usize] =
                self.client_counts[peer.client_tag as usize].saturating_sub(1);
    ...
    fn expire(&mut self, now_secs: u32, timeout_secs: u32) {
        ...
        for tag in expired_tags.drain(..) {
            self.client_counts[tag as usize] =
                self.client_counts[tag as usize].saturating_sub(1);
        }
