...
write_env() {
    printf "监听地址 [%s]: " "$DEFAULT_LISTEN"
    read LISTEN || true
    LISTEN=${LISTEN:-$DEFAULT_LISTEN}

    printf "上报间隔秒数 [%s]: " "$DEFAULT_INTERVAL"
    read INTERVAL || true
    INTERVAL=${INTERVAL:-$DEFAULT_INTERVAL}

    printf "Peer 超时秒数 [%s]: " "$DEFAULT_TIMEOUT"
    read TIMEOUT || true
...
case "${1:-menu}" in
    ...
    menu) menu ;;
    *)
        echo "用法：sh install-linux.sh [menu|install|uninstall|start|stop|restart|status|config]"
        exit 1
        ;;
esac
