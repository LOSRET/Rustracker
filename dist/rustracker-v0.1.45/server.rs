...
async fn stats(State(state): State<AppState>) -> Json<StatsResponse> {
    ...
    let history = trends.record(now, &snapshot);
    let client_data = trends.record_clients(now, &snapshot.clients);
    drop(trends);
    Json(StatsResponse::from_snapshot(snapshot, history, client_data))
}
...
struct StatsResponse {
    ...
    client_history: Vec<ClientTrendPoint>,
}
...
impl StatsResponse {
    fn from_snapshot(snapshot: TrackerSnapshot, history: Vec<TrendPointResponse>, client_data: ClientTrendData) -> Self {
        Self {
            ...
            history,
            top_clients: client_data.top_clients,
            client_history: client_data.history,
        }
    }
}
...
impl TrendStore {
    ...
    fn record_clients(&mut self, now: u64, clients: &[(u8, u64)]) -> ClientTrendData {
        ...
        // Derive top 10 from the LATEST snapshot
        let latest = self.client_points.last().map(|(_, d)| d.as_slice()).unwrap_or(&[]);
        let mut sorted = latest.to_vec();
        sorted.sort_by(|a, b| b.1.cmp(&a.1));
        let top: Vec<u8> = sorted.iter().take(TOP_CLIENT_COUNT).map(|(t, _)| *t).collect();

        // Build filled history for top clients
        let num = top.len();
        ...
        while timestamp <= bucket {
            ...
            history.push(ClientTrendPoint { timestamp, counts });
            timestamp = timestamp.saturating_add(Self::SAMPLE_SECS);
        }
